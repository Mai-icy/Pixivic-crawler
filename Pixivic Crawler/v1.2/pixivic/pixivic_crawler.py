...
class PixCrawler(object):
    ...
    def _get_page_img_url_list(self, page: int, keyword_or_date_or_id: str, mode: str, is_filter: bool) -> list:
        ...
        if mode == 'search':
            ...
            url = 'https://pix.ipv4.host/ranks?page=%d&date=%s&mode=day&pageSize=30' % (page, keyword_or_date_or_id)
        elif mode == 'artistId':
        ...
        page_img_url_list = []
        if json_res == {'message': '搜索结果获取成功'} or json_res == {'message': "获取画师画作列表成功"} \
                or json_res == {'message': '获取排行成功'}:  # 该对应页面没有图片
            return []
    ...
    def get_art_id_url_list(self, art_id: str, mode: str) -> list:
        ...
        if mode == 'illustsId':
            ...
            response = requests.get(url, headers=fake_header)
            json_res = json.loads(response.text)
            if json_res == {"message": "画作不存在或为限制级图片"}:
                return []
