...
INI_PATH = 'config\\conf.ini'


def configuration_ini(threading_num: int, num: int, token: str, path: str, is_recommend: bool, is_filter: bool):
...
def read_ini() -> dict:
    ...
    if os.path.exists(INI_PATH):
        ...
        is_filter = bool(config["advance_set"]["is_recommend"])
        res_dict = {'threads_num': threads_num, 'token': token, 'num': num,
                    'path': path, 'is_recommend': is_recommend, 'is_filter': is_filter}
        return res_dict
    else:
        return {}
...
if __name__ == '__main__':
    configuration_ini(4, 200, '123123', '1231231', True, True)
    print(read_ini())
