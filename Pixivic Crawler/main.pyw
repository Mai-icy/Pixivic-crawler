...
class MainUi(QtWidgets.QMainWindow):
    ...
    def start_button_event(self):  # 搜索模式下的开始按钮 Start button in search mode
        ...
        self.folder_name = MainUi.filter_folder_name(self.folder_name)
        if all([path, keyword]):
            ...
            if self.res_num == 0:
                ...
                for i in range(thread_num):
                    ...
                    t = WorkThread()
                    t.start()
                    t.signal.connect(self._thread_download_done)
...
class ChildUi(QtWidgets.QWidget):
    ...
    def refresh_captcha(self):  # 刷新验证码
